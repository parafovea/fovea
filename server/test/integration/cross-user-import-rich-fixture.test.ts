...
describe('cross-user import of a rich real-world export (regression for #100)', () => {
  ...
  it('every imported summary, claim, and annotation resolves its references on the importer side', async () => {
    ...
    // The fixture carries 136 summaries, 621 claims, and 9 annotations, so the
    // post-import API walk fires one /api/personas/:id lookup per summary and
    // one /api/summaries/:id/claims lookup per summary plus the world reads.
    // On the v0.3.x line every request goes through both v0.2.0's CASL
    // ability check and v0.3.0's Clean Architecture layering, which is
    // per-call work the v0.1.x line did not have, so vitest's default 5s
    // per-test budget is too tight. The test typically lands well under 60s
    // end-to-end; 90s gives headroom for the build-app cold start under the
    // Clean Architecture indirection.
  }, 90_000)
})
