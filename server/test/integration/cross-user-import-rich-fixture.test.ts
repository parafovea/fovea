...
describe('cross-user import of a rich real-world export (regression for #100)', () => {
  ...
  it('every imported summary, claim, and annotation resolves its references on the importer side', async () => {
    ...
    // On the v0.2.x line every request goes through the CASL ability check,
    // which is per-call work the v0.1.x line did not have, so vitest's default
    // 5s per-test budget is too tight. The test typically lands well under 30s
    // end-to-end; 60s gives headroom for the build-app cold start.
  }, 60_000)
})
