...
describe('cross-user import of a real Fovea export', () => {
  ...
  it('two users importing the same export each see only their own copy with resolved entity names', async () => {
    ...
    // Regression for the "Persona <uuid> not found" surface wgantt reported
    // on the reopened cross-user import bug: walk GET /api/personas/:id with
    // the summary's personaId and require a 200. The Edit Video Summary
    // dialog opens this exact endpoint to populate its persona dropdown, so
    // a 404 here is what produced the user-visible "Persona ... not found"
    // banner. The previous test only asserted the summary row carried *a*
    // personaId without verifying it dereferenced; under cross-user import
    // the summary.personaId must point at the regenerated persona, not at
    // the foreign exporter-side persona uuid that no longer exists on the
    // importer's side.
