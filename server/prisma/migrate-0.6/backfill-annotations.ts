...
import { requiredJson, type StepStats } from './helpers.js'
...
export async function backfillAnnotation(
  ...
  // Native: exact per-keyframe source values embed in the anchor's keyframe
  // features (no separate stash bag); the inverse reconstructs from the anchor
  // alone. Mirrors the current runtime write (annotation-bridge).
  const anchor = boundingBoxSequenceToSpatioTemporalAnchor(frames, {
    frameRate,
    videoWidth: width,
    videoHeight: height,
  })
