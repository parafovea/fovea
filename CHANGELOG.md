...
#### Tour Catalogue Expansion and Narration Tightening

- Tour 3 "Grow your ontology from Wikidata" now covers BOTH manual type creation AND Wikidata import in a single walkthrough so the visitor sees the contrast directly: steps 1-2 take the manual-entry path (`type-editor-mode-manual` then `type-editor-mode-wikidata`); steps 3-8 continue the existing Wikidata flow. Added missing `data-tour-id` anchors on `shared/ModeSelector.tsx` (manual / copy / wikidata).
- Tour 5 "The world layer" now walks all four world-object editors (entity / location / event / time) with narration that calls out what is distinctive about each (entity binds a TYPE to a thing, location adds coordinates and a map pin, event has start/end + role bindings the entity editor does not, time has the start/end/fuzzy controls). Added a previously missing event-instance step.
- Tour 2 "Building a persona's ontology" relation-type-editor narration is tightened to explicitly contrast the source-types and target-types pickers against the entity / event / role type editors.
- Tours 3, 6, 7 narrate the accept-some / reject-some / inline-edit / split-compound-claim editing loop the demo is built around. Tour 3 augmenter-results gets two extra steps (accept the close-but-not-quite "Ball grab" suggestion and rename it to lowercase `ball-grab`, reject the four distractors). Tour 6 candidates-list + tracking-results-panel get three extra steps (accept the two high-confidence container boxes, reject the two spurious low-confidence boxes, snap to the realistic general type `container` with Wikidata Q987767, re-anchor the tracker at the flagged frame 214). Tour 7 transcript-viewer + video-summary-editor + claims-extraction-dialog get four extra steps (inline-edit `snatched` to `grabbed`, flip the speaker chip, correct `above the right-field line` to `behind home plate`, split the non-atomic compound claim into three atomic rows).
- Tour 7 gains a two-step prelude visiting the on-demand Transcribe Audio button + TranscriptPanel dialog before the saved-summary `audio-config-panel` + `transcript-viewer` flow so the new surface lands inside the same tour the visitor walks for summaries and claims.
- House style: em-dashes scrubbed out of every user-facing `narration` / `recap` / `description` / `title` string across the twelve tour scripts; replaced with semicolons or hyphens contextually. Same sweep over the new `src/tours/content/types.ts` + `src/tours/content/microvent.ts` + `src/mocks/tourDemo/handlers.ts` + `src/mocks/tourDemo/browser.ts` comments and console.info so the next admin reading the JSON for their own deployment does not pick up the punctuation that the house style banned.

#### Type Strictness on Model-Service Inbound Routes

- `model-service/src/infrastructure/adapters/inbound/fastapi/routes/transcribe.py`: `result: Any` is replaced by `result: TranscriptionResult`; model is typed `AudioTranscriptionLoader` via `cast`. Empty-string language is normalised to `None` so the auto-detect path is not rejected by faster-whisper.
- `model-service/src/infrastructure/adapters/inbound/fastapi/routes/diarize.py`: model is typed via a local `Protocol` (`_DiarizationModel`) with the single `diarize(audio_path) -> DiarizationResult` contract, so the route does not couple to `PyannoteLoader` as a concrete class. `raw_segments: list[dict[str, Any]]` is replaced by `result: DiarizationResult` (typed dataclass with `SpeakerSegment` members).
...
- Comprehensive Docusaurus reorganization at `docs/docs/`: industry-standard split into Tutorial / Guide / Concepts / Reference / Operations / Project; orphan markdown at the doc root deleted or moved into the published tree; the Operations section grows from a single demo-deployment runbook to six pages covering production deployment, monitoring (OTel + Prometheus + Grafana), backup-and-restore, upgrades, troubleshooting, and the existing demo-fovea-deployment runbook. Docusaurus serves at `docs.fovea.video` (GitHub Pages CNAME); the marketing landing stays at `fovea.video`.
...
- House style: em-dashes (`—` / `–`) removed from every doc and replaced with semicolons / commas / hyphens; all docs use American spelling (`organize`, `behavior`, `color`, `catalog`, `license`, `flavor`, `whilst -> while`, ...) so a future contributor cannot tell from the prose which contributor wrote which page. The `guide/tour-catalogue.md` file is renamed to `guide/tour-catalog.md` (with the sidebar and cross-page links updated).
