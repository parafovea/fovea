...
- An idempotent backfill migrates each deployment's existing 0.5.11 videos, annotations, summaries, ontologies, world state, and claims into the native layers shapes (`server/prisma/backfill/`, a `runner.ts` over per-surface modules keyed through a shared `id-map`, with `backfill-expression-summary-native.ts`, `backfill-video-annotations-native.ts`, `backfill-ontology-native.ts`, `backfill-world-native.ts`, and `backfill-claims-native.ts`), so 0.6.0 carries pre-existing data forward rather than starting empty; a companion verifier reconstructs each migrated annotation's bounding-box sequence and asserts it against the original bit-for-bit (`server/prisma/backfill/verify.ts`).
