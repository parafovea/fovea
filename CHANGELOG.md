...
- Close the resurfacing of #92 (anonymous visitor and logged-in user on refresh briefly see the Video Browser shell instead of the login page or their normal view). Pre-fix `useSession.ts` had no `else` branch on a non-2xx `/api/config` response, so a 5xx under heavy load silently skipped `setConfig` and left `appConfig` at `null` and `mode` at the auth-store's `'single-user'` initial-state default. The pre-fix `ProtectedRoute` then gated only on `isLoading`, and the subsequent `/api/auth/me` 401 fired `logoutSuccess()` which cleared `isLoading` to `false`, allowing the route to fall through to its children (Video Browser) because `mode === 'multi-user'` was not yet true. The fix tightens `ProtectedRoute` to also hold the loading screen while `appConfig === null` (the only authoritative signal that the server's actual deployment mode is known) and wraps `/api/config` in a bounded exponential-backoff retry across `[500, 1000, 2000, 4000, 8000]ms` so a transient failure no longer permanently strands the user on the loading screen. A `cancelled` ref in the effect prevents post-unmount state writes during the wait.
- Close #122 (33+ vitest tests fail with `TypeError: Cannot read properties of null (reading 'useContext')` in `annotation-tool/`). A leftover `node_modules/.pnpm/react@18.3.1/...` tree from cross-branch experimentation produced two physical React copies; React's dispatcher singleton lives on whichever copy `react-dom` resolves against, and any hook caller (including MUI's `useTheme`) resolved against the OTHER copy read a `null` dispatcher. `rm -rf node_modules && npm install` rebuilds the tree under npm's flat hoist (this branch's checked-in lockfile is `package-lock.json`, not pnpm) so a single React resolves across the render tree. `vitest.config.ts` carries the rule structurally so a future re-migration to pnpm cannot reintroduce the split: `resolve.alias` pins `react` / `react-dom` / `react/jsx-runtime` to the symlinked sibling of `react-dom`'s realpath (the only React `react-dom` can be peer-linked against); `resolve.dedupe` includes both packages and their JSX runtimes; `optimizeDeps.include` pre-bundles MUI / Emotion through the same alias chain so vitest's CJS interop does not hand them off to Node's resolver; and `test.server.deps.inline` keeps `@mui/*` / `@emotion/*` / `@base-ui/*` out of vitest's externalisation path.

### Added

- Regression test in `annotation-tool/src/App.test.tsx` reproduces the `appConfig === null` + `isLoading === false` state directly and asserts the loading screen renders with no protected content visible. Without the new gate this test renders the Video Browser shell.
- Regression suite in `server/test/integration/cross-user-import-rich-fixture.test.ts` against `server/test/fixtures/cross-user-import-rich-export.jsonl` (the richest of the seven annotator exports uploaded to #121, carrying 20 personas / 20 ontologies / 79 entities / 136 summaries across ~96 distinct videos / 621 claims / 9 object annotations — structurally far richer than the existing single-persona `cross-user-import-real-export.jsonl`, which is too thin to expose any remap crosstalk between concurrent persona / summary / claim mappings). The test imports the fixture into a fresh user and walks four assertions sourced directly from the screenshot on the reopened #100: (a) every imported summary's `personaId` dereferences via `GET /api/personas/:id` with a 200 (a 404 here is the user-visible 'Persona <uuid> not found' banner in the Edit Video Summary dialog), (b) every dereferenced persona is owned by the importer (cross-checked against `GET /api/personas`), (c) no `summary.personaId` equals one of the original exporter-side persona ids (i.e. the remap actually rewrote it, not just preserved it), (d) every imported claim's `summaryId` resolves to a summary owned by the importer (the second 404 surface in the screenshot — claims pointing at the foreign summary id), with round-trip claim and annotation counts matching the fixture exactly. The suite passes on this tip in ~4.2s, validating the v0.1.10 structure-agnostic remap against a realistic payload.
