...
The 0.6.0 release is a single coordinated landing that moves Fovea's annotations onto one unified model. The video bounding-box tracks, ASR transcripts, ontologies, world objects, and claims that five bespoke shapes held in 0.5.11 now persist as `pub.layers.*` records, with one panproto-generated type layer shared by the TypeScript server, the React frontend, and the Python model-service. The FOVEA↔layers integration is fully native — every surface is modeled on native layers primitives with no sidecar blobs — and runs in production through `@panproto/core` bidirectional lenses generated from the vendored 0.10.0 lexicons. Alongside it, the guided-tour engine becomes data-authored: a tour is a validated document over a typed anchor catalog rather than code. The release is breaking on both fronts. 0.6.0 retains the legacy `Annotation`/`WorldState`/`Ontology`/`Claim`/`ClaimRelation` tables so a one-time `migrate-0.6` copy can move their data into the layers store while both schemas coexist (the expand and migrate phases of an expand, migrate, contract upgrade); a later release drops those tables behind a guard that refuses while they hold rows the copy has not verified. The previous tour engine's internals are replaced. The 0.6 backend copies a 0.5.x deployment's data on its first start and refuses to start if the copy does not verify, so self-hosted operators upgrade with a backup and a normal deploy; see the "Upgrading 0.5 to 0.6" operations guide (`docs/docs/operations/upgrading-0.5-to-0.6.md`) for the exact route.

### Added

#### Unified layers annotation model

- Video, audio, and text annotation persist through one layers-shaped store (`Expression`/`Segmentation`/`AnnotationLayer`/`LayersAnnotation`/`GraphNode`/`GraphEdge`/`TypeDef`), replacing the per-concept `Annotation`/`WorldState`/`Ontology`/`Claim` tables. A `BoundingBoxSequence` round-trips bit-exactly through a `spatioTemporalAnchor`, so the video timeline is unchanged. The `@fovea/layers-schema` workspace package vendors the generated `pub.layers.*` types and JSON Schema as the one annotation vocabulary for the server and frontend, and a pnpm catalog consolidates the shared dependency versions the server and frontend pin against.
- The store is served through an `/api/layers/*` route surface (`server/src/routes/layers/` — `expressions`, `annotation-layers`, `graph`, `ontologies`, `corpora`, `media`, `video-annotations`, `interchange`), backed by dedicated repositories (`ExpressionRepository`, `GraphRepository`, `CorpusRepository`, `MediaRepository`, `InterchangeRepository`) and services carrying their own CASL rules. The `video-annotations` route lists only the video annotation subkinds, so claim text spans stored on the same video `Expression` do not surface there as empty-geometry entries (`server/src/routes/layers/video-annotations.ts`). A `layers-conversion-service` (`server/src/services/layers-conversion-service.ts`) converts frame-based bounding-box tracks to and from a `spatioTemporalAnchor` server-side, so the timeline UI keeps its bounding-box-sequence view-model.
- A document-annotation workspace adds token-standoff span labeling with discontiguous spans and typed span relations whose directedness follows the ontology relation type — a relation type flagged symmetric renders as an undirected edge; spans link to ontology types or world objects, and video-associated text (tweets, ASR transcripts) annotates in a side panel. The discontiguous-span mechanics live in a side-effect-free library (`annotation-tool/src/lib/spans/` — `tokenSpanMap`, `selectionToSegments`, `spanGeometry`, `spanColors`, `adapters`) that maps a DOM text selection to token-index segments and lays out span highlights and relation arcs independently of React, over which `SpanAnnotator` (`annotation-tool/src/components/document/`) layers keyboard-driven selection, a label picker, and a `RelationArcOverlay`, with span and relation state held in a Zustand store (`createSpanAnnotatorStore`). A non-primary overlapping span is selectable as a relation endpoint from its side-panel row, and a CASL-gated read-only span viewer wires through the document and video-text hosts (`annotation-tool/src/components/document/`), so a viewer without edit rights sees the spans and relations without a labeling affordance.
- Corpora import and export as layers JSONL, so annotations interchange losslessly with any `pub.layers.*` consumer; the flows drive from the UI through an `ExportLayersDialog` and an `ImportCorpusDialog` (`annotation-tool/src/components/data-management/`) wired via a `useCorpus` query layer to the `/api/layers/interchange` route, so an operator moves a corpus in or out without a CLI.

#### Bidirectional panproto lenses and native catalog collections

- All four FOVEA↔layers surfaces — video, world, claim, and ontology — translate in production as `@panproto/core` bidirectional lenses. A panproto registry (`server/src/services/layers-lens/panproto-registry.ts`) loads each `pub.layers.*` record as an ATProto `BuiltSchema` with cross-file refs resolved and emits each FOVEA view-model as a JSON Schema derived from its Zod definition; the per-surface lens documents (`server/src/services/layers-lens/{video,world,claim,ontology}-lens.ts`) carry the expressible value transforms, and a record-to-Prisma-row adapter assembles the multi-record output each surface persists.
- Each surface pairs a forward `getJson` with a backward reverse-lens for reconstruction: the forward direction carries a non-injective confidence projection (0–1 ↔ 0–1000), so it is not an isomorphism and a reverse-oriented lens — not the forward lens's `put` — is the stored-row reconstruction. Both directions satisfy the `checkGetPut`/`checkPutGet` round-trip laws, and the read and write paths of every surface run through them (`server/src/services/layers-bridge/{annotation,world,claim,ontology}-bridge.ts`).
- FOVEA world collections persist on the native `pub.layers.catalog.collection` and `catalog.membership` tables (`CatalogCollection`/`CatalogMembership`, mapped `catalog_collections`/`catalog_memberships` in `server/prisma/schema.prisma`) across the world forward, persistence, and reconstruction paths (`server/src/services/layers-lens/world-collection-catalog.ts`, `world-lens.ts`, `server/src/services/layers-bridge/world-store.ts`), with `clusterSet` retained only for video tracks and coreference.

#### Model-service layers codec

- The model-service projects its own inference outputs — ASR transcripts, object detections, object tracks, video summaries, extracted claims, and ontology types — onto canonical `pub.layers.*` records through a per-domain `dx.Lens` bridge (`model-service/src/infrastructure/adapters/outbound/layers/lenses/{transcript,detection,tracking,summary,claims,ontology}.py`), each lens reconstructing the source DTO from native fields alone. A framework-neutral `ILayersCodec` port (`model-service/src/application/ports/outbound/layers_codec.py`) speaks in normalized `(local_id, nsid, value_json)` fragments and stays lairs-free, so the application layer depends on it without pulling the record-model stack; the concrete adapter (`.../outbound/layers/codec.py`, `_convert.py`, `lairs_codec_adapter.py`) binds it to the `lairs` record model. `POST /api/layers/import` decodes a fovea envelope to `pub.layers.*` records and `POST /api/layers/export` serializes posted records as layers JSONL (`.../inbound/fastapi/routes/layers.py`, `.../outbound/layers/corpus_io.py`), so the model-service interchanges losslessly with any layers consumer.
- When the `lairs` extras are not installed the codec port binds to a null codec (`model-service/src/infrastructure/adapters/outbound/layers/null_codec.py`) whose methods raise, and the import/export routes report `501 Not Implemented` rather than a 500, so a deployment built without the record-model stack still boots and answers every other route (`.../config/container.py`, `.../inbound/fastapi/dependencies.py`).

#### Data-authored tour engine

- The guided-tour engine is data-authored ([#180](https://github.com/parafovea/fovea/pull/180)): `annotation-tool/src/tours/engine/anchorCatalog.ts` declares every tour-addressable UI surface once, as a typed entry recording its description, the route or dialog it lives on, whether it is conditional, and the `reachedBy` controls that open it. `AnchorId` is `keyof typeof anchorCatalog`, so a tour step or seeder that names an anchor the catalog does not define fails to compile. A component publishes its element with `useTourAnchor(id)` (`annotation-tool/src/tours/engine/anchorRegistry.tsx`), which registers the live node in an `AnchorRegistry` and tags it `data-tour-anchor` for the inspector and tests.
- A tour is a single zod schema (`annotation-tool/src/tours/engine/tourSchema.ts`); `parseTour` validates a document and narrows it to the `Tour` type, so a malformed tour fails loudly with a field-anchored message. `annotation-tool/src/tours/content/tourLoader.ts` assembles the catalogue a deployment runs: the first-party tours merged with an administrator's tours served from `public/tours/` (a manifest at `/tours/index.json`). An override replaces the built-in sharing its id, a new id is appended, and a tour marked `enabled: false` is dropped, so an administrator tailors the tour set without touching application code.
- `annotation-tool/src/tours/engine/TourRunner.tsx` drives a tour as a state machine that, for each step, navigates to the step's route, runs its `driver` capability to put the workspace into the state the step needs, resolves the anchor by clicking the catalog `reachedBy` controls, and simulates the expected action. On an interactive step the runner renders an anchor-registry `SpotlightOverlay` (`annotation-tool/src/tours/engine/`) whose backdrop is click-through, so the guided click, draw, or scrub the step asks for reaches the target element beneath the overlay. Capabilities are named side effects (`capabilities.ts`, `seeders.ts`) a step references by name, so a step declares the state it needs rather than scripting how to reach it, and the runner emits a `TourEvent` for every transition (`events.ts`) while staying policy-free.
- The provider folds the runner's event stream into an analytics log (`annotation-tool/src/tours/menu/tourTelemetry.ts`): a `started`, a `step_viewed` per step carrying its dwell time, and a terminal `completed` or `abandoned`. It persists a lightweight `fovea.tour.cursor` so a reload resumes in place, the step card carries a Pause control, and an author-mode anchor inspector (`annotation-tool/src/tours/engine/AnchorInspector.tsx`) lists the anchors currently on the page so a tour can be authored by reading ids off the running UI.
- A built-in document-annotation tour walks the document browser, span annotator, token selection, span labeling, and the relations panel, driving each reachable state through new demo-document seeder capabilities (`annotation-tool/src/tours/scripts/document-annotation.ts`, `annotation-tool/src/tours/engine/seeders.ts`); the layers document, span, and relation surfaces are named entries in the anchor catalog (`anchorCatalog.ts`), and the relation-drawing step is narrated because the instance-scoped span store cannot be reached from a capability.

### Changed

#### Native layers integration without sidecars

- The FOVEA-to-`pub.layers.*` integration models every surface on native layers primitives rather than verbatim sidecar blobs: ontology descriptions are stand-off gloss `Expression`s, world state is native `GraphNode`/`GraphEdge`/`ClusterSet` rows with typed temporal and spatial value-objects, claims are span-bearing annotations, and video keyframes carry native `spatioTemporalAnchor`s — so `features.foveaOntology`, `properties.foveaWorld`, `properties.foveaClaim`, and the per-keyframe `fovea.*` keys exist in no stored record (`server/src/services/layers-lens/{world,claim,ontology,video}-lens.ts`, `server/src/services/layers-bridge/{world,claim,ontology,annotation}-bridge.ts`, `server/src/services/layers-bridge/world-store.ts`).
- Every world entity, event, location, time, and collection reconstructs from native `GraphNode`, `GraphEdge`, and `ClusterSet` rows plus typed temporal and spatial value-objects, with deep temporal structure — vagueness, deictic reference, recurrence, and granularity — carried on `temporalModifier` and `temporalEntity` fields rather than flat feature keys, and the interpretation persona held as an `argumentRef` (`server/src/services/layers-lens/world-lens.ts`, `server/src/services/layers-bridge/world-store.ts`, `server/src/services/layers-id-map.ts`, `server/src/services/world-state-service.ts`).
- Claims and claim-relations reconstruct onto native span annotations bearing an `argumentRef` claimer and graph edges, with the claim-relation edge-id derivation carried in a named helper (`server/src/services/layers-lens/claim-lens.ts`, `server/src/services/layers-bridge/claim-bridge.ts`, `server/src/services/claim-service.ts`). Video track identity and provenance home onto a per-video track `ClusterSet` and the grouping layer's `sourceMethod`, persona-scoped world-instance annotations materialize as native world `GraphNode`s so the detection query lists them, and world-instance node identity is scoped to its owner so same-label instances across videos collapse to a single node (`server/src/services/layers-lens/{world,video}-lens.ts`, `model-service/src/infrastructure/adapters/outbound/layers/lenses/{detection,tracking}.py`).
- Every model-service FOVEA↔layers lens drops its complement to `None` and reconstructs the stored row from native fields alone, so no lens carries a sidecar complement, and model inference telemetry is dropped rather than sidecared into the annotation (`model-service/src/infrastructure/adapters/outbound/layers/lenses/{claims,detection,ontology,summary,tracking,transcript}.py`, `model-service/src/infrastructure/adapters/outbound/layers/codec.py`).
- The `LayersAnnotation` anchor column is nullable, and a world-denoting annotation persists it as a genuine SQL `NULL` through an omitted field rather than an empty-object or JSON-`null` literal (`server/prisma/schema.prisma`, `server/prisma/migrations/*`, `server/src/repositories/ExpressionRepository.ts`).
- A `migrate-0.6` CLI copies each deployment's existing 0.5.11 videos, annotations, summaries, ontologies, world state, and claims into the native layers shapes (`server/prisma/migrate-0.6/`, a `runner.ts` over per-domain modules keyed through a shared `id-map`, driven by `cli.ts` with `auto`, `preflight`, `export`, `dry-run`, `migrate`, `verify`, `status`, and `rollback` subcommands exposed as `npm run migrate:0.6:*` and bundled into the backend image as `prisma/migrate-0.6/cli.cjs`), so 0.6.0 carries pre-existing data forward rather than starting empty. Each domain is copied through the application's own writer (`writeVideoAnnotation`, `mergeWorldObjects`, `writeOntologyAggregate`, `writeClaim`, `writeClaimRelation`) so the panproto lens transform is never reimplemented, and the copy is idempotent and resumable. A companion verifier reconstructs each migrated annotation's bounding-box sequence against the canonical 0.6 projection and checks content fidelity for world objects, ontology types, and claims through the application's own backward read path, recording a passing verify in a durable `_layers_migration_state` marker (`server/prisma/migrate-0.6/verify.ts`, `state.ts`). The guarded contract migration that a later release ships drops the legacy tables only when they are empty or that marker reads verified (`server/prisma/migrate-0.6/drop-legacy.template.sql`). The backend's start command runs `auto` after `prisma migrate deploy`: it copies and verifies only when the five 0.5 tables hold rows and no verified copy is recorded, so fresh installs and already-migrated deployments start untouched, and `migrate` refuses to re-run after a verified copy without `--force` because a re-run would revert edits made in 0.6. The demo deploy workflow takes a Postgres backup before migrating (keeping the last five) and runs the same copy step before starting the application.
- A partial update to corpus or membership metadata merges rather than wholesale-replacing, so facets the caller does not restate survive the write (`server/src/services/corpora-service.ts`).

#### Bidirectional panproto lens runtime

- The `pub.layers.*` TypeScript types are generated directly from the vendored 0.10.0 lexicons (`packages/layers-schema/lexicons/pub/layers/`) by a dependency-free Node port of the idiolect-codegen emitter (`packages/layers-schema/scripts/gen-types.mjs`, with `regenerate.sh` and `extract-components.mjs` and the input tracked at `json-schema/.source-commit`), so the checked-in lexicons are the single source of truth for the shared annotation vocabulary — including the `catalog/collection.json` and `catalog/membership.json` record schemas.
- `@panproto/core` (0.74.3) backs the lens runtime (`server/package.json`): `putJson` applies a step inverse across the JSON boundary and preserves collection order in the backward direction, so the ordered-children round-trip laws are sound evidence, and the WASM binding surfaces `put_without_complement` and `isIsomorphism`.
- The Docker build compiles the `@fovea/layers-schema` workspace package before the dependent server and annotation-tool images (`server/Dockerfile`, `annotation-tool/Dockerfile`), a pnpm `prepare` script builds that package before the server and frontend type-check so a fresh install type-checks against the generated `pub.layers.*` types, and the server-side model-service contract types are generated from the service's OpenAPI with the recursive `JsonValue` contract flattened out of a self-referential `openapi-typescript` alias.

#### Model-service on didactic and Python 3.14

- The model-service is rebuilt on didactic: every domain entity, DTO, and FastAPI request body is a `dx.Model`, the architecture dispatch is a `dx.TaggedUnion`, config is didactic-settings, and no first-party Pydantic remains (FastAPI keeps it transitively through the didactic-fastapi bridge). The service runs on Python 3.14, gating the inference backends that lack cp314 support — `vllm`, `sglang`, `openai-whisper`, `bitsandbytes`, and `sam-2` — behind a `python_version` marker in `model-service/pyproject.toml` so the remaining ML stack resolves onto torch 2.13 from cp314 wheels; the container source-builds `llama-cpp-python` with an added build toolchain (`model-service/Dockerfile`), and the CI, docs, and security jobs run on Python 3.14 (`.github/workflows/{ci,docs,security}.yml`).

#### Data-authored tour engine

- `annotation-tool/src/components/ui/dialog.tsx` reads whether a tour is active and, while one runs, renders non-modal and ignores outside-press and focus-out close reasons, so the tour's step card stays reachable over an open dialog and the engine can drive a surface inside it. Outside a tour, dialog behavior is unchanged.
- The `Tabs`, `Dialog`, `Card`, `ToggleGroup`, `Select`, and `Alert` UI wrappers forward refs to their underlying DOM node, so a `useTourAnchor` ref attached to one of these compound components resolves to a real element the spotlight can address (`annotation-tool/src/components/ui/{tabs,dialog,card,toggle-group,select,alert}.tsx`).

### Removed

- The `/api/annotations` route and the legacy `Annotation`, `WorldState`, `Ontology`, and `Claim` Prisma models are removed; every read and write path — video tracks, ontologies, world objects, claims, and summaries — goes through the layers store, so a client on the old annotations endpoint no longer resolves (`server/src/routes/layers/index.ts`, `server/prisma/schema.prisma`).
- The four hand-rolled FOVEA↔layers oracle mappers (`server/src/services/{video-annotation-mapper,world-layers-mapper,claim-layers-mapper,ontology-layers-mapper}.ts`) and the migration-only oracle-parity harness (`server/src/services/layers-lens/oracle-parity.ts`) are gone now that production runs on the lens; their surviving domain types and helpers live in `video-annotation-shared.ts`, `world-model.ts`, `claim-model.ts`, and `ontology-model.ts`.
- The model-service's `schemas/` and `mappers/` layers are gone — the per-concept FastAPI request/response schema modules (`model-service/src/infrastructure/adapters/inbound/fastapi/schemas/{claims,common,detection,ontology,reasoning,summarization,tracking}.py`) and their DTO-to-schema mappers — so one `dx.Model` per concept reaches the routes through a `didactic-fastapi` bridge that builds the request bodies dynamically (`.../inbound/fastapi/dx_bodies.py`).
...
#### Native layers integration

- World-authored nodes are distinguished from video-denotation stubs, so pruning the world leaves a live annotation's link to the node it denotes intact (`server/src/services/layers-lens/world-lens.ts`, `server/src/services/layers-bridge/world-store.ts`).
- World-object deletion, persona type-deletion, persona deletion, and claim re-extraction each run as one all-or-nothing unit by threading a Prisma interactive transaction through tx-aware layers-bridge writers, so the annotation, gloss, and world rows a given operation touches commit together or not at all (`server/src/services/persona-service.ts`, `server/src/services/layers-bridge/{world-bridge,annotation-bridge,claim-bridge,ontology-bridge,util}.ts`, `server/src/queues/setup.ts`, `server/src/services/world-state-service.ts`).
- The layers interchange import/export contract is aligned and its cross-tenant authorization closed: the video text-expression endpoint gates on source read access, world and ontology writes are atomic and version-guarded over the correct collection-member fields, graph edge endpoints validate against a concrete object-reference schema, and a foreign-id annotation collision maps to an explicit conflict response (`server/src/repositories/InterchangeRepository.ts`, `server/src/services/{interchange-service,text-expression-service,world-state-service,graph-service,claim-service}.ts`, `server/src/routes/layers/{graph,media,video-annotations}.ts`).
- Imported expression foreign keys enforce parent-ownership, the annotation column imports with an FK-safe `denotesNode` reference, the video ASR transcript materializes from the first caller-readable summary, and system administrators are exempt from the project-membership gate on persona creation (`server/src/repositories/{ExpressionRepository,InterchangeRepository}.ts`, `server/src/services/{text-expression-service,persona-service,claim-service}.ts`, `server/src/services/layers-id-map.ts`).

#### Data-authored tour engine

- `annotation-tool/src/tours/content/tourLoader.ts` treats an HTML response to `/tours/index.json` as "no manifest" and returns the built-in tours, so a deployment that ships no administrator manifest — where the single-page-app history fallback serves `index.html` with a 200 — loads its catalogue cleanly, while a manifest the server genuinely serves as JSON still validates loudly.
- The E2E build compiles the `@fovea/layers-schema` workspace package and bundles the layers-importing seed into a self-contained CommonJS file, and the export/import E2E selectors are disambiguated from the Export Corpus and Import Corpus buttons.

### Testing

- The layers store round-trips end-to-end: the server backfill carries a round-trip verifier and fixture-backed suite that reconstruct each annotation's bounding-box sequence bit-for-bit (`server/prisma/backfill/__tests__/backfill.test.ts`, `verify.ts`), and a model-service corpus I/O test loads a `pub.layers.*` JSONL corpus, re-emits it, and asserts fidelity (`model-service/test/interop/layers/test_corpus_io.py`, `fixtures/sample_corpus.jsonl`).
- Each FOVEA↔layers surface gains a native round-trip suite alongside cross-user isolation and authorization regression coverage (`server/src/services/__tests__/video-annotation-mapper.test.ts`, `server/test/services/{world-layers-mapper,claim-layers-mapper,import-cross-user}.test.ts`, `server/test/routes/{layers-annotation-null-anchor,layers-corpora-merge,layers-authz-regression}.test.ts`, `server/test/integration/{layers-interchange-cross-user,multi-user-isolation,persona-type-deletion}.test.ts`).
- A layers interop suite exercises each domain lens and the codec end to end, asserting the `pub.layers.*` projection round-trips through the transcript, detection, tracking, summary, claims, and ontology lenses (`model-service/test/interop/layers/`), and the torch-gated ML tests run with no skips on Python 3.14 (`model-service/test/loaders/test_detection_loader.py`, `model-service/test/config/test_catalog_dispatch_invariant.py`, `model-service/test/test_app_boot.py`).
- Every built-in tour has a regression spec that launches it through the engine and asserts each step's anchor resolves and the tour completes (`annotation-tool/test/e2e/regression/tours/`), run against the real backend and the MP4 video corpus, and an engine smoke suite (`annotation-tool/test/e2e/smoke/tour-engine.spec.ts`) covers the runner, spotlight, step card, telemetry, the resume cursor, pause and resume, focus, and keyboard handling.
...
- Extracted the **projects** domain into a `ProjectRepository` (pure Prisma data access, including the create-project-with-owner-membership transaction) and a `ProjectService` (orchestration, authorization, and response mapping), reducing `server/src/routes/projects.ts` from 1072 lines with 32 direct `prisma.*` calls to 581 thin lines with zero, following the personas pattern.
- Migrated project authorization onto CASL, the same engine every other domain already uses. The projects routes previously enforced access with bespoke inline checks (`['project_owner','project_manager'].includes(myRole)` role-string comparisons and a `request.user.isAdmin` boolean); `Project` is now a first-class CASL subject in `server/src/lib/abilities.ts`, so reads, updates, deletes, member management, and project creation resolve through the same `RolePermission` matrix and request-scoped `ability` as personas, claims, world state, and video. The database already granted these project permissions (`project_owner` → update/delete/manage_members, `project_manager` → update/manage_members, every project role → read, `group_owner`/`group_admin` → create) and `manage_members` was an already-defined-but-unused CASL action, so the policy is unchanged - this removes a duplicated, divergent authorization path rather than changing who may do what. The group-scoped create permission is now correctly limited to the group a candidate project belongs to (a group administrator could previously be granted creation against any group). One deliberate consistency change: a project administrator is now recognized through the CASL system role (`systemRole === 'system_admin'`), matching every other domain, rather than the separate `isAdmin` boolean column; seeded and provisioned administrators carry both, so no real administrator is affected.
- Added `GET /api/projects/:projectId/assignable-users` (authorized by `manage_members`), returning the users who are not already members of a project. The "Add member" picker on the project detail page previously called the admin-only `/api/admin/users` endpoint, so it returned a 403 and showed an empty list to project owners and managers who were not system administrators; it now reads from the scoped endpoint and works for anyone who can manage the project's members.
- Extracted the **claims** domain into a `ClaimRepository` (pure Prisma data access) and a `ClaimService` (orchestration, the relocated CASL authorization, and response mapping), reducing `server/src/routes/claims.ts` from 1559 lines with 33 direct `prisma.*` calls to 783 thin lines with zero. The claims routes already authorized through CASL, so every `can(...)` / `accessibleBy(...)` decision is preserved verbatim, along with the denormalized `claimsJson` rebuild, the claim-extraction and claim-synthesis queue jobs, the auto-created empty summary, and the subclaim cascade - a structural refactor with no behavior change.
- Extracted the **world-state** domain into a `WorldStateRepository` (pure Prisma data access) and a `WorldStateService` (orchestration, the relocated CASL authorization, the demo / single-user read handling, and the object-reference gloss cleanup), reducing `server/src/routes/world.ts` from 1148 lines with 27 direct `prisma.*` calls to 372 thin lines with zero. The world routes already authorized through CASL, so every `can(...)` / `accessibleBy(...)` decision is preserved verbatim - including the existence-privacy distinction between forbidden and not-found, the anonymous / demo-mode read widening, the single-user default-user resolution, and the entity / event / time object-reference-to-text gloss cleanup on deletion - a structural refactor with no behavior change.
- Split the 2232-line `server/src/services/import-handler.ts` import state machine into focused modules under `server/src/services/import/`: `line-parser.ts` (line parsing + validation), `dependency-graph.ts`, `conflict-resolver.ts` (conflict detection, resolution, id remapping, and cross-user resolution), `entity-importers.ts` (the per-type `EntityImporter` collaborator), and `types.ts` - leaving `import-handler.ts` a 517-line orchestrator. The pure parsing / graph / conflict logic became standalone functions; the per-type database writes moved to an `EntityImporter` that receives the transaction client on each call, so the entire import still runs inside the same single interactive `$transaction` (same `maxWait` / `timeout`, same write order). `routes/import.ts` and the public `ImportHandler` API are unchanged; behavior is identical.

#### Cross-Service Contract Generation (OpenAPI)

- The backend now emits its OpenAPI 3.x specification to a committed `server/openapi.json` (`pnpm --filter @fovea/server gen:openapi`), and the frontend generates its API types from that spec with `openapi-typescript` (`pnpm --filter @fovea/annotation-tool gen:api-types` → `src/api/generated/openapi.ts`). The hand-maintained object-detection and ontology-augmentation request/response types in `annotation-tool/src/api/client.ts` are deleted and re-exported from `src/api/generated/contracts.ts`, where they are derived from the generated spec, so the frontend's `DetectionResponse` / `AugmentationResponse` / `OntologySuggestion` shapes now track the server schema by construction rather than by hand. A new `contract-drift` CI job regenerates the spec and the frontend types and fails the build when either is stale, making the cross-service detection/ontology drift structurally impossible.
- Fixed a latent ontology-augmentation casing bug: `OntologyAugmenter.tsx` carried its own `AugmentationResponse` type declaring `persona_id` / `target_category` (snake_case) while the API actually returns `personaId` / `targetCategory` (camelCase). The component body already read the camelCase fields, so the mismatch was confined to the (now-deleted) type and its test fixtures, both corrected.

#### Model-Service Contract Layer Decoupling

- Removed the eager submodule re-exports from the model-service `src/infrastructure/**/__init__.py` chain, so importing a Pydantic contract DTO (a request/response schema) no longer transitively loads the entire outbound ML adapter tree (`adapters → outbound → video → processor → cv2`). Importing a schema now pulls only that module and its real ancestors; `cv2`/`torch` load only when something actually uses the video processor. This removes an import-time-side-effect anti-pattern and lets the cross-service contract layer be imported (and emitted) without the ML stack — the foundation for an upcoming model-service contract pipeline. No call sites changed (every consumer already imported via concrete deep paths); the full model-service test suite (1235 tests) and the assembled app are unchanged.
- Raised the `opencv-python` floor to `>=4.10` so the resolver can never select the numpy-1-ABI `4.9.x`, which fails to import against the `numpy>=2` the audio stack (`pyannote-audio>=4`) requires. Fresh installs already resolved to a numpy-2-compatible build; this pins the requirement explicitly to prevent an ABI regression on a stale lock.

#### Server↔Model-Service Contract Pipeline

- The model-service now emits a committed OpenAPI 3.1 contract (`model-service/openapi.json`) for the shapes the server consumes (detection, ontology augmentation, claim extraction, summary synthesis, summarize), and the server generates TypeScript types from it (`pnpm --filter @fovea/server gen:model-service-types` → `src/lib/model-service/contract.ts`). Generation is **ML-free** — `model-service/scripts/gen_contract_spec.py` imports only the Pydantic request/response models (not the FastAPI app) and asserts `cv2`/`torch` never load — so the spec is deterministic and the CI gate does not need the heavy ML install. This is the producer-owned counterpart to the server→frontend OpenAPI generation: the model-service owns the shapes it produces; the server tracks them by generation rather than by hand.
- `server/src/lib/model-service/contract-assertions.ts` adds **compile-time compatibility assertions** between the generated producer types and the server's hand-written model-service-facing interfaces (`ModelClaimExtractionResponse`, `ModelSynthesisResponse`, `ModelSummarizeResponse`/`Request`, and the detection/ontology request+response expectations). If the model-service drops, renames, or retypes a field the server reads, regenerating the contract makes `tsc` fail with an error whose `offendingKeys` names the exact field — a server↔model-service drift becomes a red build, not a runtime surprise. A new blocking `contract-drift-model-service` CI job (Contracts / Model Service Drift Check) regenerates the spec and the generated types and fails on any diff. This surfaced and corrected one latent annotation drift (`ModelSummarizeResponse.key_frames` was typed `number[]` but the producer sends `KeyFrame` objects, stored verbatim into a JSON column — a type-only fix). `docs/docs/development/cross-service-contracts.md` documents the single workflow for adding or changing a cross-service endpoint.

#### Model-Service Loader Files Split One-Class-Per-File

- The two monolithic model-service loader modules — `vlm/loader.py` (1158 lines, 6 VLM loaders) and `detection/loader.py` (771 lines, 7 detection loaders) — are split into `loaders/` subpackages with one loader per file plus a shared `loaders/base.py` (the config/enums, the abstract base, the architecture-keyed registry, and the factory). Each former `loader.py` is kept as a thin aggregator that re-exports the public surface and imports every per-loader module for its `@registry.register(...)` side effect, so registration still runs on import and no consumer import path changes. Pure reorganization — no loader logic, registry, factory dispatch, or architecture class changed; the registered architectures (9 VLM, 7 detection PyTorch + 3 ONNX), the architecture↔loader invariant test, `mypy`, and the full suite are unchanged.

#### Demo-Mode Read Widening Consolidated

- Demo mode (`FOVEA_DEMO_MODE`) exposes system-seeded content to anonymous/all users on the public-demo/booth deployment by widening read access; that widening was duplicated ad hoc across `routes/annotations.ts`, `routes/summaries.ts`, `routes/persona-preferences.ts`, `services/persona-service.ts`, `services/video-access-service.ts`, and `services/world-state-service.ts`, each with a different shape, so what demo mode exposed was hard to audit. It is now consolidated into one module, `server/src/lib/demo-rbac.ts` — the single source of truth for the per-subject demo read scopes (`{ source: { startsWith: 'demo-fixture' } }` for annotations, `{ isSystemGenerated: true }` for personas) and the demo read predicates (videos, summaries, world state, system-persona access). Each helper gates on `isDemoModeEnabled()` internally, so no route or service scatters that flag check anymore (a grep of `routes/` + `services/` for `isDemoModeEnabled` is now empty). Pure consolidation: every site's effective query/decision is byte-for-byte preserved — demo mode exposes the same content, and with demo off a self-hoster's per-user RBAC is exactly as before.

#### Annotation Workspace Logic Extracted into Hooks

- The 1403-line `annotation-tool/src/components/annotation/AnnotationWorkspace.tsx` is decomposed into three cohesive custom hooks under `components/annotation/hooks/`: `useAnnotationDialogs` (the editor/summary/detection/transcript dialog state and the transcript-request flow), `useSummaryFlow` (the claim timestamp-capture wiring and the draft-claim auto-open), and `useAnnotationState` (the playback state, the keyframe/edit handlers, and the auto-save setup). The component drops to a thin composition of the three hooks plus its render. Pure move — state, effects, dependency arrays, handler logic, and render output are unchanged (the keyframe handlers still pass the freshly-mutated array to `forceSave`, and the auto-save change-detection snapshot is byte-identical), so behavior is preserved.
...
- `POST /api/annotations` now accepts an optional client-generated `id` and is idempotent on it: when the id already exists, the existing annotation is updated in place (HTTP 200) instead of a duplicate being minted (HTTP 201 for a genuine create). The update path authorizes against the **existing** row via the same instance-level `can('update', subject('Annotation', …))` gate as `PUT /api/annotations/:id`, and only writes mutable fields — identity columns (`videoId`, `createdByUserId`, owner) are never repointed by an id. A concurrent-create race (two requests with the same new id) is collapsed to a single row via a `P2002` fallback to the update path. This is the backend half of the autosave-duplicates fix: the annotation workspace re-sends each box with its stable client id, so a lagged auto-save re-POST of an already-persisted box no longer creates a second row. (Sending no `id` preserves the previous create-with-fresh-id behavior, so existing clients are unaffected; the client wiring + the auto-save loop fix land alongside the workspace decomposition.)

#### Annotation Auto-Save No Longer Loops or Duplicates Boxes (Client Half)

- The annotation workspace auto-save no longer fires in a perpetual ~1/sec loop while a video with annotations is idle, and editing a box no longer spawns a duplicate. Two client fixes complete the issue (with the idempotent backend create above): (1) `useAutoSave` gained an optional `getComparisonSnapshot` so its change-detection can ignore server-managed fields; the workspace strips `updatedAt`/`createdAt` before comparing, so the post-save refetch echoing a new `updatedAt` no longer looks like a change and re-arms the save. (2) The create request now sends the annotation's stable client `id` (`transformFrontendToBackend`), so a freshly-created box keeps that id instead of diverging from a server-minted one — and a lagged re-save is an idempotent update, not a duplicate. Other `useAutoSave` callers are unaffected (the comparison default is unchanged).
- Fixed a related silent data-loss bug the loop had been masking: keyframe edits (add / remove / update / move, interpolation-segment changes) and box drag / resize / nudge wrote the change to the local query cache and then forced a save in the **same tick**, before the cache update propagated back into the auto-save's `data` — so the forced save persisted the **pre-edit** array and dropped the edit. The save succeeded (no error), so once the masking loop was removed the edit would have been lost on reload. The keyframe/box mutation hooks now return the freshly-computed array and the edit handlers pass it to `forceSave(dataOverride)`, which persists exactly that snapshot (and updates the change baseline so the later propagation does not re-fire a save). The end-to-end persistence test now asserts the edited keyframe genuinely survives a reload, rather than that a box is merely visible.
