...
`Persona.userId` is the ownership column. CASL's ability builder
uses it when compiling per-row conditions; an own-only reader sees
only their own personas (plus system-generated ones), and a
project member additionally sees personas with the matching
`projectId`. The optional `Persona.projectId` makes a persona
project-scoped. See [Guides > Projects](projects.md) and
