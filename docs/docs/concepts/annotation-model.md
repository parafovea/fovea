...
## Where the userId column matters

`Annotation.userId` was added in `20260310000000_add_annotation_userid`
to support user-scoped object annotations (object annotations
have an optional `personaId`, so the persona-side ownership check
does not apply). v0.1.8 made the listing endpoint
(`GET /api/annotations/:videoId`) return only annotations whose
`personaId` belongs to the requester or whose `userId` is the
requester. The listing was previously unscoped, which is what
let cross-user imports show up as duplicate rows.
